...
class list_entry_test(BaseTestClass):
    ...
    def test_add_entry(self):
        ...
        response = self.app.post("/api/v1/bucketlists/23/entry",
                        data= entry_name, headers=self.header,
                        content_type=self.mime_type)
        print (response)
        response_data =json.loads(response.data)

        response_control = self.app.post("/api/v1/bucketlists/23/entry",\
                                         data= entry_name, headers=self.header,
                                         content_type=self.mime_type)
        ...
        self.assertEqual("Entry successfully added to the list",
    ...
    def tests_add_item_wrong_inputs(self):
        ...
        blank_response = self.app.post("/api/v1/bucketlists/1/entry",
                                        data=blank, headers=self.header,
                                        content_type=self.mime_type)
        empty_response = self.app.post("/api/v1/bucketlists/1/entry",
                                        data=empty, headers=self.header,
                                        content_type = self.mime_type)
        space_response = self.app.post("/api/v1/bucketlists/1/entry",
        ...
        self.assertEqual("No blank entries allowed",
    ...
    def test_delete_item(self):
        ...
        response = self.app.delete("/api/v1/bucketlists/1/entries/1",
                                    headers=self.header)
        response_data = json.loads(response.data)
        ...
        entry_unavailable  = self.app.delete("/api/v1/bucketlists/1/entries/1",
                                                headers=self.header)
    ...
    def test_update_item(self):
        ...
        valid_response= self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data= valid_data,
                                        headers=self.header,
                                        content_type=self.mime_type)
        blank_response = self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data= blank_data,
                                        headers= self.header,
                                        content_type=self.mime_type)
        invalid_response= self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data= invalid_status,
                                        headers=self.header,
                                        content_type=self.mime_type)
        invalid_url_response= self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data = valid_data,
                                        headers =self.header,
                                        content_type=self.mime_type)
        empty_name_response= self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data= empty_name,
                                        headers=self.header,
                                        content_type=self.mime_type)
        space_name_response= self.app.put("/api/v1/bucketlists/1/entries/2",
                                        data= space_name,
                                        headers=self.header,
                                        content_type=self.mime_type)
