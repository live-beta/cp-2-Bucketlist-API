...
class BucketlistsTest(BaseTestClass):
    ...
    def test_authorisation_required(self):
        ...
        post_item = self.app.post("/api/v1/bucketlists/1/entry")
        post_item_data = json.loads(post_item.data)
        self.assertListEqual([401,401], [get_bucket.status_code, post_item.status_code])
        self.assertListEqual([Bucketlist.query.count(), Entry.query.count()],[1,1])
    ...
    def test_successfully_add_view_bucketlist(self):
        ...
        response = self.app.post("/api/v1/bucketlists", data = data, headers= self.header, content_type=self.mime_type)
        ...
        response_final =self.app.get("api/v1/bucketlists",headers=self.header)
    ...
    def test_inputs_required_to_post(self):
        ...
        response = self.app.post("api/v1/bucketlists", data = no_det, headers=self.header, content_type= self.mime_type)
        response_data = json.loads(response.data)
        final_data = Bucketlist.query.count()

        self.assertListEqual("Bucketlist name is required", response_data["message"]["name"])
        bl_name = json.dumps({"name": ""})

        bl_name_response = self.app.post("api/v1/bucketlists", data= bl_name,headers=self.header,content_type=self.mime_type)
        ...
        self.assertListEqual([400,400,400],[response.status_code,bl_name_response.status_code,sp_name_response.status_code])
        self.assertListEqual([0,0,0],[final_data -initial,new_bl -initial, new_sp -initial])

    def test_deleting_bucketlist(self):
        "Test that bucketlist can be viewed and deletes"
        initial_bc_count = Bucketlist.query.count()

        initial_entry_count =Entry.query.count()

        deleted_bucket =self.app.delete("/api/v1/bucketlists/1", headers=self.header)
        deleted_bucket_data = json.loads(deleted_bucket.data)

        new_bc_count = Bucketlist.query.count()
        new_entry_count = Entry.query.count()

        self.assertEqual(deleted_bucket.status_code,200)
        self.assertListEqual([1,1],[initial_bc_count - new_bc_count, initial_entry_count -new_entry_count])
        self.assertIn("testlist has been deleted", deleted_bucket["message"])

        # When one attempts to delete a non existent bucketlist
        no_bucket_list_delete = self.app.delete("/api/v1/bucketlists/1", headers= self.header)
        no_bucket_list_delete_data = json.loads(no_bucket_list_delete.data)
        self.assertEqual(404, no_bucket_list_delete.status_code)
        self.assertIn("bucjetlist not found", no_bucket_list_delete_data["message"])
    ...
    def test_update_bucketlist(self):
        ...
        no_bucket = self.app.put("/api/v1/bucketlists/1", data=bl_data,headers =self.header, content_type= self.mime_type)
        no_bucket_data =json.loads(no_bucket.data)

        blank = self.app.put("/api/v1/bucketlists/1", data =data_empty, headers=self.header, content_type=self.mime_type)
        blank_data = json.loads(blank.data)

        noname =self.app.put("/api/v1/bucketlists/1", data=data_noname, headers=self.header, content_type= self.mime_type)
        noname_data = json.loads(noname.data)
        name_space =self.app.put("/api/v1/bucketlists/1", data= data_space, headers= self.header,content_type=self.mime_type)
        name_space_data = json.loads(name_space.data)

        self.assertListEqual([200,400,400,400,404],[valid.status_code,blank.status_code,noname.status_code,name_space.status_code,no_bucket.status_code])
    ...
    def test_user_can_not_access_other_users_buckets(self):
        ...
        bucket = self.app.post("api/v1/bucketlists", data = new_bucket, headers=headerbob, content_type = self.mime_type)

        self.assertEqual(201, bucket.status_code)
        self.assertEqual(2,Bucketlist.query.count())

        # Alice's attempt to access
        alice_bucket = self.app.get("api/v1/bucketlists/2", headers= self.header)
    ...
    def test_bad_route(self):
        ...
        bad_post_response = self.app.post("/api/v1/bucketlists/1", data=name, headers=self.header, content_type=self.mime_type)
        bad_put_response = self.app.put("/api/v1/bucketlists", data = name, headers=self.header, content_type =self.mime_type)
        ...
        self.assertEqual("bad request", bad_post_response_data["message"])
    ...
    def test_search_bucketlist(self):
        ...
        search =self.app.get("/api/v1/bucketlists?q=list", headers=self.header)
        ...
        missing = self.app.get("/api/v1/bucketlists?q=missing", headers =self.header)
    ...
    def test_pagination_limit_for_bucketlist(self):
        ...
        self.app.post("/api/v1/bucketlists", data= name_2, headers=self.header, content_type= self.mime_type)

        page_1 = self.app.get("/api/v1/bucketlists?page=1&limit=1",headers=self.header)
        ...
        page_all =self.app.get("/api/v1/bucketlists?page=1&limit=3", headers=self.header)
        page_all_data = json.loads(page_all.data)

        self.assertListEqual([200,200,200],[page_1.status_code,page_2.status_code,page_3.status_code])
